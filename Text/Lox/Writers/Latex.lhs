Transforming Formulas to LaTeX (`Text.Lox.Writers.LaTeX`)
---------------------------------------------------------

Writing formulas back to a target format from its abstract form is mostly
straight-forward. All that needs to be done is to recursively *compile* the
abstract syntax tree into a concrete string representation, combining
constituents with the appropriate operator symbols and ultimately representing
atoms as strings (usually their identifier).
...
> termAsLatex (Operation op t1 t2) =
>     termAsLatex t1 ++ " " ++ latexOperator op ++ " " ++ termAsLatex t2
...
> application id ts =
>     id:'(':(foldl1 (\ x y -> x ++ ", " ++ y) (map termAsLatex ts)) ++ ")"
